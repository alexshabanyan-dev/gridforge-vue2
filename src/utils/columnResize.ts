...
export function findChangedColumn(
  ...
  for (const columnId in newSizing) {
    const oldSize =
      previousSizing[columnId] ?? currentSizingForCompare[columnId] ?? 0;
...
export function applyMinSizeConstraint(
  ...
  const minSize =
    (column.columnDef as ColumnDef<TableRow, unknown>).minSize ??
    DEFAULT_MIN_SIZE;
 ...
 */
export function getFullColumnSizing(
  table: GridforgeTableInstance,
...
export function createFitModeResizeHandler(
  ...
  return (
    ...
    originalSetColumnSizing: (updater: Updater<ColumnSizingState>) => void,
  ) => {
    if (layout !== 'fit') {
    ...
    const { columnId: changedColumnId, delta: initialDelta } =
      findChangedColumn(
        table,
        newSizing,
        previousSizing,
    ...
    if (changedColumnId) {
      ...
      if (changedColumn) {
        const minSize =
          (changedColumn.columnDef as ColumnDef<TableRow, unknown>).minSize ??
          DEFAULT_MIN_SIZE;
        if (newSizing[changedColumnId] < minSize) {
          newSizing[changedColumnId] = minSize;
          delta =
            minSize -
            (previousSizing[changedColumnId] ?? changedColumn.getSize());
        }
      }
    }
    ...
    if (changedColumnId && Math.abs(delta) > DELTA_THRESHOLD) {
      ...
      const changedIndex = allColumns.findIndex(
        (col) => col.id === changedColumnId,
      );
      ...
      if (changedIndex >= 0 && changedIndex < allColumns.length - 1) {
        ...
        const nextMinSize =
          (nextColumn.columnDef as ColumnDef<TableRow, unknown>).minSize ??
          DEFAULT_MIN_SIZE;
        const adjustedNextSize = Math.max(
          nextMinSize,
          nextColumnCurrentSize - delta,
        );
